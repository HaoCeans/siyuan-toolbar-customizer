...
使用 `text:` 匹配宿主界面的实际文本。旧格式仍然兼容，但只匹配指定文字：

```text
text:复制块引用
```

对于会随思源语言设置变化的菜单项，推荐使用语言候选格式：

```text
text:zh-CN=思源手机端增强|en=SiYuan Mobile Enhancer
```

插件会优先尝试当前界面语言，找不到时回退到其他候选。语言标签支持 `zh-CN`、`zh` 和 `en`；每个候选文本都必须非空，并且必须与页面中显示的实际文字完全一致。该语法只适用于宿主界面固定文本；用户填写的文档名、搜索词等业务内容不要自动翻译。
...
```
toolbarMore
text:zh-CN=立即同步|en=Sync now
```
**说明**：打开手机端更多菜单后，按当前思源语言匹配“立即同步”或“Sync now”；当前语言找不到时会自动回退到另一候选。
...
每个步骤成功后默认等待 **200ms**，让界面有时间响应和渲染。

**自定义间隔（v3.8.3+）**：在序列中单独写一行时间，即可覆盖后续步骤的等待时间（取最近一次）：
- `200ms` / `800ms` / `1s` / `1.5s` —— 支持 `ms` / `s` 单位，不写单位默认为毫秒（如 `800` = 800ms）
...
上面的例子：点击 `barPlugins` → 等待 **1 秒** → 再点击 `text:思源手机端增强`（默认是 200ms，此处被 `1s` 覆盖）。

适合慢动画/慢加载场景：在需要多等待的步骤前插入一行 `1000ms` 或 `2s` 即可，无需全局调参。
