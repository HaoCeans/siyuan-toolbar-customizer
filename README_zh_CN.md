...
### v3.8.11 — 弹窗里能识别标签/引用了 + 模板可追加到日记顶部

> 💡 一键记事弹窗里打 `#` 不出标签候选、打 `[[` 不出引用候选？手写模板按钮只能把内容追加到日记末尾？升。

**① 一键记事弹窗放开标签与引用候选**
- 弹窗里输入 `#` 现在会弹出已有标签候选；输入 `[[` / `【【` / `（（` / `((` 会弹出引用候选，选中文档或块后插入真正的引用节点
- 原因：插件此前用 `display:none !important` 把思源的候选面板（`.protyle-hint`）连预览、上传、工具条一起关掉了，思源自己摘掉隐藏类也顶不过来；现在只放开这一个面板，其余元素照旧隐藏
- 同时收窄候选注册表：这个小小的弹窗里去掉斜杠菜单（`/`、`、`）与 emoji（`:`），只保留标签和引用；面板宽度改为随弹窗收窄（不再固定 320px 起），层级提到发送/取消按钮之上
- 覆盖手机端与电脑端「页内」记事弹窗；电脑端「块格式独立窗口」本来就没有隐藏该面板，不受影响

**② 手写模板插入：可追加到每日笔记顶部**
- 「②手写模板插入【简单】」填了笔记本 ID 后，新增开关「⬆️ 追加到每日笔记顶部（关闭＝追加到末尾）」
- 打开后内容写到当日日记最前面，关闭则维持原来的追加到末尾
- 电脑端与手机端配置界面均已提供；旧按钮没有这个字段，一律按末尾处理，行为不变
