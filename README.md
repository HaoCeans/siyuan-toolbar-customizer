...
### v3.7.7 — 修复模拟点击误伤 + 性能优化 🎯

> 💡 **v3.7.6 的菜单位置修复误伤了「⑤自动化模拟点击」的 barPlugins 等全局按钮，建议升。**

#### 🐛 修了啥

**1. 模拟点击的 barPlugins / text:xxx 点不到了**
- v3.7.6 加的编辑器限定查找逻辑太宽，把 `barPlugins`、`text:思源手机端增强` 等全局元素也拦住了
- 现在改为白名单：**只有 `more` 和 `doc` 两个按钮走新逻辑**，其他所有模拟点击完全不变

#### ⚡ 顺手优化了

**2. Electron 子窗口 hash 轮询 50ms → 200ms**
- 插件设置弹窗里 50ms 查一次 hash 太费 CPU，降到 200ms，省 75% 的活

**3. 桌面端胶囊标签页轮询 1s → 3s + 自动停止**
- eventBus 已经能处理标签页切换了，轮询改为 3 秒一次且 90 秒后自动停

**4. 滚动隐藏状态切换统一走缓存**
- 显示/隐藏工具栏时统一用缓存查 DOM，少做重复查询
