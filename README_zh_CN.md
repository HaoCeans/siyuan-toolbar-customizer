...
### v3.7.3 — 修复电脑端块格式弹窗尺寸记忆丢失 + 隐藏清理定时器泄漏 + 卸载窗口匹配不可靠

> 🎯 **本版本主要修复电脑端块格式一键记事的多个遗留问题，建议块格式用户升级。**

## 🐛 修复

**1. 电脑端块格式弹窗关闭后窗口尺寸记忆丢失**
- × 按钮关闭走 `win.destroy()` 只触发 `closed` 事件，`saveBounds` 因 `win.isDestroyed()` 为 true 而跳过保存
- 新增 `_lastBounds` 缓存兜底，窗口销毁时用最后已知位置写入 localStorage
- 连带修复：重启思源后弹窗回到居中默认尺寸的问题

**2. 电脑端块格式弹窗隐藏后销毁导致孤立草稿块**
- `closed` 事件处理器漏清 `_hideTimer`（隐藏后 5 秒自动清理定时器），窗口销毁后定时器仍触发，在日记中创建无人清理的空块
- 与 `close` 事件处理器保持一致，增加 `clearTimeout(_hideTimer)`

**3. 电脑端块格式弹窗卸载时 `getTitle()` 窗口匹配不可靠**
- 思源内部覆盖窗口标题后 `getTitle()` 返回的不是 `'⚡ 快捷记事'`，导致卸载/重启时僵尸窗口残留
- 改用 `__qn_block_window` 标记匹配，与 `destroyAllBlockWindows` 统一

## ✨ 优化

**4. 电脑端块格式弹窗默认尺寸调整**
- 600×500 → **300×300**，位置改为**屏幕靠右居中**（右侧贴边 40px、垂直居中）
