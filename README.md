...
> 💡 **如果你用「⑤自动化模拟点击」功能，建议升，v3.7.6 的修复误伤了你。**

#### 🐛 修了啥

**1. 模拟点击的 barPlugins / text:xxx 点不到了**
- v3.7.6 修more/doc的时候多管闲事，把 `barPlugins`、`text:思源手机端增强` 这些全局按钮也拦住了
- 现在改回白名单：**只有 `more` 和 `doc` 走新逻辑**，其他模拟点击该咋跑还咋跑

#### ⚡ 顺手优化

- **插件设置弹窗** hash 轮询从 50ms 降到 200ms，CPU 少干 75%
- **桌面端胶囊**标签页轮询 3 秒一次，90 秒后自动停，不空转
- **滚动隐藏**状态切换统一走缓存，少查几次 DOM
