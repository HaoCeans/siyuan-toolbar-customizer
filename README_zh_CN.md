...
### v3.7.3 — 修了几个电脑端块格式弹窗的老毛病

> 🎯 **如果你在用电脑端块格式一键记事，建议升级。**

## 🐛 修复

**1. 重启后弹窗尺寸位置又变回默认了？修好了**
- 以前关弹窗的时候尺寸没记下来，下次打开又回到默认大小
- 现在关窗时妥妥记住你调好的宽高位置，重启也不丢了

**2. 隐藏弹窗后销毁，可能会在日记里留下空白块？修好了**
- 以前有个隐藏后自动清理的定时器，如果定时器还没跑完就把弹窗关了，它还会在后台偷偷执行，在日记里插一个空块
- 现在关窗时定时器也会一并清掉，不留尾巴

**3. 卸载插件或重启后可能残留僵尸窗口？修好了**
- 卸载插件时清理窗口的匹配方式不够靠谱，有时候清不掉，窗口变成"僵尸"留在那
- 现在改用更靠谱的方式匹配，卸载重启都能清干净

## ✨ 优化

**4. 弹窗默认尺寸改小了，位置也更顺手**
- 600×500 → **300×300**，默认跑到**屏幕右边居中**（不挡你中间的内容）
