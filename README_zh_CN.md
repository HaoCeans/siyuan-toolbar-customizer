...
- 🧹 **移除复杂检测逻辑** - 删除频繁切换、长后台、冻结恢复等检测（减少约 350 行代码）
...
| 项目 | 改进说明 |
|------|---------|
| **触发逻辑** | 从"切前台触发"改为"切后台触发"，更符合用户习惯 |
| **代码简化** | 从 ~1800 行优化到 ~1450 行（-19% 代码量） |
| **检测机制** | 从 7 种状态检测简化为 1 种触发 + 1 种清理 |
| **可靠性** | 单点触发避免多检测点竞争，提升稳定性 |
