...
> 💡 **如果你开了「更多按钮隐藏」或「文档菜单按钮隐藏」，并且用默认的「更多」「打开菜单」按钮，建议升。**

#### 🐛 修了啥

**1. 点了「更多」菜单却跑偏了**
- 隐藏按钮是用 CSS 把按钮缩没了（`scale(0)` + 宽度 0），但按钮其实还在 DOM 里
- 默认的「更多」「打开菜单」按钮会去点那个被藏起来的原生按钮，让思源弹出菜单
- 思源弹菜单时拿 `getBoundingClientRect()` 算位置——宽度为 0 的时候 `右边 = 左边`，菜单位置就往左歪了
- **修复**：点之前临时把按钮尺寸恢复回来，让思源算对位置，点完立刻重新藏好

**2. 底部胶囊模式下菜单跑到了右下角**
- 胶囊容器用了 `transform: translateX(-50%)` 来居中
- CSS 有个坑：祖先元素用了 `transform` 之后，`position: fixed` 的子元素不再相对视口定位，而是相对那个元素
- 之前试过用 `position: fixed` 去挪隐藏按钮的位置，结果坐标全算错了
- **修复**：不改 `position`，只恢复尺寸——按钮本来就在工具栏里位置是对的

**3. 切换文档后菜单跑到了左上角**
- 切文档时旧编辑器只是隐藏了（加了 `fn__none` 类），DOM 没删
- 查找按钮时 `querySelector` 按文档顺序找第一个，可能找到的是隐藏编辑器里的按钮
- 隐藏元素的 `getBoundingClientRect()` 返回 `{left:0, top:0}` → 菜单就在左上角弹出来
- **修复**：找到按钮后先确认它和点下去的按钮在同一个编辑器里，不是的话去正确编辑器里重新找
